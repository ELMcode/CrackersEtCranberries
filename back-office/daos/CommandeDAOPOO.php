...
class CommandeDAO
{
     ...
     */
    public function selectAll($pdo): array
    {
        ...
        try {
            ...
            for ($i = 0; $i < count($list); $i++) {
                $commande = new Commande($list[$i]["id_commande"], $list[$i]["id_client"], $list[$i]["qte_commande"], $list[$i]["date_commande"], $list[$i]["statut_commande"], (float) $list[$i]["montant_total_commande"]);
                $result[] = $commande;
            }
    ...
    //Déclaration de la méthode INSERT :: input : un objet commande , output : un numérique entier
    /**
     * @param PDO $pdo
