...
// @return int
//
// On déclare une fonction nommée selectOneByPseudoAndMdp qui a des paramètres pour la connexion BD de type PDO et le p
//et le mdp de typ chaine
// La fonction renvoie un nombre
function selectOneByPseudoAndMdp(PDO $pdo, string $emailClient, string $mdpClient): int
{
    ...
    $sql = "SELECT COUNT(*) FROM client WHERE email_client=? AND mdp_client=?";
    // Variable pour compiler (-> binaire)
    $cursor = $pdo->prepare($sql);
    // Valorise le 1e paramètre (donc le 1e ?)
    $cursor->bindValue(1, $emailClient);
    $cursor->bindValue(2, $mdpClient);
    // execute
    $cursor->execute();
